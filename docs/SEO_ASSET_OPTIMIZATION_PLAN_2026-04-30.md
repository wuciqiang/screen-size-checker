...
### 2026-04-30：Phase 3 Hub 页重构进度

- 已完成：`/hub/ips-vs-tn-vs-va-gaming` 英文、中文、德语、西语、葡语、法语版本重写。
- 处理原则：没有直接把 2025 改成 2026，而是删除旧的年份化断言、职业选手比例和易过期型号推荐，改为基于 VESA DisplayHDR、RTINGS、TechSpot、TFTCentral 等可核验来源的面板选择逻辑。
- 页面新定位：从普通 listicle 调整为“2026 游戏显示器面板选择决策页”，覆盖 Fast IPS、VA、Mini-LED VA、OLED/QD-OLED/WOLED、TN 的适用场景、风险和购买检查清单。
- 多语言处理：各语言版本都加入本地自然表达和单位/购买语境，不再只做英文直译。
- 验证状态：已通过 `npm run build` 和 `npm test`；sitemap 仍为 229 个 canonical URL，核心内部链接和语言导航测试通过。

### 2026-04-30：`/hub/hdr-gaming-monitor-guide` 重写完成

- 已完成：英文、中文、德语、西语、葡语、法语版本重写，并同步更新 `build/pages-config.json`。
- 处理原则：没有把旧稿的 2025 直接替换成 2026，而是删除“HDR400 一律假 HDR”“固定价位/型号推荐”等过度简化内容，改为基于 VESA DisplayHDR CTS 1.2、认证过渡期、OLED/Mini-LED 实测维度和 Google helpful content 的判断流程。
- 页面新定位：从“最佳 HDR 显示器列表”改为“真实 HDR 验证与选购决策页”，覆盖 DisplayHDR/True Black、HDR10 与真实显示能力区别、局域控光、亮度窗口、色彩体积、延迟、OLED 烧屏风险和 Windows/游戏校准。
- 多语言处理：中文补充电商语境下“支持 HDR 信号”和“真实 HDR 效果”的区分；德语、西语、葡语、法语版本按各自购买语境重写，不再保留旧稿机翻和乱码。
- 验证状态：已通过 `npm run build` 和 `npm test`；sitemap 仍为 229 个 canonical URL，内部链接 847/847 可访问。

### 2026-04-30：`/hub/gaming-monitor-size-guide` 选择器化完成

- 已完成：英文、中文、德语、西语、葡语、法语版本重写，并同步更新 `build/pages-config.json`。
- 处理原则：没有机械把日期改到 2026，而是删除旧稿里难以持续核验的职业选手比例、固定显卡/价格组合和手写“最后更新”字段，改为基于 PPI、桌面深度、观看距离、游戏类型、刷新率目标和人体工学的决策选择器。
- 页面新定位：从长篇指南改为“游戏显示器尺寸选择器”，首屏直接给 24-25 英寸、27 英寸、32 英寸、超宽屏的适用场景，并提供桌深/距离/GPU/游戏类型矩阵。
- 多语言处理：各语言版本按本地用户的自然购买语境重写，统一使用 `{{lang_prefix}}` 内链，避免非英语页面误链到英文 canonical。
- 验证状态：已通过 `npm run build` 和 `npm test`；sitemap 仍为 229 个 canonical URL，内部链接 847/847 可访问。

### 2026-04-30：Hub 多语言硬编码英文内链清理

- 已完成：扫描并修正非英语 hub 内容中 `](/hub/...)` 形式的硬编码英文内链，统一改为 `]({{lang_prefix}}/hub/...)`。
- 处理范围：`1440p-vs-4k-gaming`、`144hz-vs-240hz-gaming`、`best-monitor-size-fps`、`curved-vs-flat-monitor-gaming`、`g-sync-vs-freesync-gaming`、`gaming-monitor-vs-tv`、`ultrawide-vs-dual-monitor-gaming` 的 zh/de/es/pt 版本。
- 处理原则：只修正跨语言跳转问题，不顺手改 2025 内容，不把旧年份强行替换为 2026。
- 验证状态：已通过 `npm run build` 和 `npm test`；非英语 hub 源文件已无 `](/hub/...)` 硬编码英文 hub 内链。

### 2026-05-01：`/hub/1080p-vs-1440p-gaming` 重写完成

- 已完成：英文、中文、德语、西语、葡语、法语版本重写。
- 处理原则：没有把 2025 直接改成 2026，而是删除旧稿中固定 FPS 表、固定整机价格、职业选手比例和手写“最后更新”等不可持续断言，改为基于像素数量、PPI、刷新率目标、GPU 等级、显存约束和游戏类型的决策页。
- 最新数据来源：核对 Steam Hardware & Software Survey 当前 Windows 主屏分辨率、GPU/VRAM 分布，以及每日更新的 PassMark GPU 等级表；页面明确说明这些数据只用于趋势判断，不承诺具体游戏 FPS。
- 页面新定位：从“1080p vs 1440p 性能/价格对比文章”改为“2026 游戏分辨率决策页”，直接回答 1080p 240Hz 与 1440p 144Hz、24 英寸与 27 英寸、电竞与综合游戏的取舍。
- 多语言处理：中文加入“2K 显示器”搜索表达和国内电商/接口检查语境；德语加入 QHD/WQHD 与桌面深度语境；西语覆盖西班牙与拉美价格/保修差异；葡语按巴西市场的价格、分期、保修语境重写；法语加入法国电商术语和售后/接口检查语境。
- 验证状态：已通过 `npm run build`、`npm test` 和 `git diff --check`；构建仍为 465/465 页面成功，sitemap 仍为 229 个 canonical URL，内部链接 847/847 可访问。

### 2026-05-01：`/hub/1440p-vs-4k-gaming` 重写完成

- 已完成：英文、中文、德语、西语、葡语、法语版本重写。
- 处理原则：没有把旧稿 2025 强行替换为 2026，而是删除固定 FPS 表、固定整机预算、职业玩家比例、无法核验用户评价和旧价格区间，改为基于 4K 相比 1440p 的 2.25 倍像素负载、PPI、显示器尺寸、GPU 等级和刷新率目标的决策页。
- 最新数据来源：核对 Steam Hardware & Software Survey 当前 Windows 主屏分辨率分布，其中 1440p 为 21.33%、4K 为 4.89%，并用 PassMark 每日更新 GPU 表作为显卡等级参考，不把它写成游戏 FPS 保证。
- 页面新定位：从“性能/成本对比文章”改为“1440p 与 4K 游戏分辨率决策页”，直接回答 27 英寸/32 英寸、1440p 240Hz/4K 144Hz、竞技/剧情/创作/主机等场景取舍。
- 多语言处理：中文补充 2K/4K 电商参数、4K 高刷接口和笔记本外接检查；德语补 QHD/WQHD/UHD 术语；西语覆盖西班牙与拉美价格/进口差异；葡语按巴西价格、分期、接口和保修语境重写；法语加入法国电商术语、SAV、退换和接口检查。
- 验证状态：已通过 `npm run build`、`npm test` 和 `git diff --check`；构建仍为 465/465 页面成功，sitemap 仍为 229 个 canonical URL，内部链接 847/847 可访问。

### 2026-05-01：`/hub/144hz-vs-240hz-gaming` 重写完成

- 已完成：英文、中文、德语、西语、葡语、法语版本重写。
- 处理原则：没有把旧稿 2025 强行替换为 2026，而是删除旧稿中无法持续核验的盲测比例、职业选手采用率、固定价格表、名人引用和“锦标赛标准设备”断言，改为基于帧时间、系统延迟链路、面板响应、VRR 标准和游戏场景的判断页。
- 数据与资料依据：使用 144Hz/240Hz/360Hz/480Hz 的稳定帧时间计算，结合 NVIDIA Reflex 对系统延迟链路的解释、VESA Adaptive-Sync Display CTS、2024 年 FPS 刷新率研究和 TestUFO 用户侧验证方法，不把这些资料写成所有玩家都必须升级 240Hz 的结论。
- 页面新定位：从“240Hz 是否值得的泛泛对比文章”改为“刷新率决策页”，直接回答 144Hz 到 240Hz 的真实收益、165/180Hz 中间档、竞技 FPS 与 3A 游戏差异、1440p 240Hz 与 4K 144Hz 的取舍。
- 多语言处理：中文补充国内电商高刷参数、笔记本接口、网吧/电竞馆试用和“2K 高刷”购买语境；德语补本地 QHD/WQHD、桌面深度和欧盟购买语境；西语覆盖西班牙与拉美保修/进口差异；葡语按巴西价格波动、接口和 arena gamer 语境重写；法语加入法国 SAV、退换、接口和 salle gaming 语境。
- 验证状态：已通过 `npm run build`、`npm test` 和 `git diff --check`；构建仍为 465/465 页面成功，sitemap 仍为 229 个 canonical URL，内部链接 847/847 可访问。

### 2026-05-01：`/hub/g-sync-vs-freesync-gaming` 重写完成

- 已完成：英文、中文、德语、西语、葡语、法语版本重写。
- 处理原则：没有把旧稿 2025 强行替换为 2026，而是删除固定价格溢价、泛化“最佳技术”结论、旧时间线包装和难以核验的性能断言，改为以 GPU、接口、VRR 范围、认证层级、HDR 和主机 HDMI VRR 为核心的决策页。
- 最新资料来源：联网核对 NVIDIA G-SYNC/G-SYNC Compatible 官方页面、NVIDIA certified monitor specs、AMD FreeSync 分级与 LFC 说明、VESA Adaptive-Sync Display CTS、HDMI VRR 官方说明；页面只使用这些资料解释认证和链路，不把标志写成绝对优劣。
- 页面新定位：从“G-SYNC vs FreeSync 技术优劣对比”改为“VRR 购买决策页”，直接回答 NVIDIA/AMD/主机/HDR/高刷新场景下应该检查哪些条件。
- 多语言处理：中文补充国内电商“兼容 G-SYNC”与原生模块的差异、笔记本外接、PS5 HDMI VRR 和退换货语境；德语补 EU/德国退换与接口检查；西语覆盖西班牙与拉美进口/保修差异；葡语按巴西价格波动、分期、保修和接口语境重写；法语加入 SAV、退换和法国电商精确型号检查。
- 验证状态：已通过 `npm run build`、`npm test` 和 `git diff --check`；构建仍为 465/465 页面成功，sitemap 仍为 229 个 canonical URL，内部链接 847/847 可访问。

### 2026-05-01：`/hub/gaming-monitor-vs-tv` 重写完成

- 已完成：英文、中文、德语、西语、葡语、法语版本重写。
- 处理原则：没有把旧稿 2025 强行替换为 2026，而是删除固定价格区间、泛化输入延迟阈值和“显示器/电视绝对更好”的旧式对比，改为按观看距离、输入方式、接口链路、主机支持、HDR 和办公比例来决策。
- 最新资料来源：联网核对 HDMI VRR、PS5 120Hz/VRR、Xbox 4K 120Hz、VESA DisplayHDR/True Black 相关官方资料；页面只用这些资料建立购买检查清单，不写无法持续维护的型号和价格表。
- 页面新定位：从“游戏显示器 vs 电视性能对比文章”改为“PC/主机/客厅/办公混合显示设备决策页”，直接回答桌面键鼠、沙发手柄、4K 120Hz、1440p 240Hz、OLED 电视当显示器等场景。
- 多语言处理：中文补充国内电商 HDMI 2.1 标注、PS5 HDMI VRR、桌深和 OLED 电视当显示器语境；德语补 EU/德国退换和接口检查；西语覆盖西班牙与拉美进口/保修；葡语按巴西价格、保修和桌面空间语境重写；法语加入 SAV、退换和法国电商端口标注检查。
- 验证状态：已通过 `npm run build`、`npm test` 和 `git diff --check`；构建仍为 465/465 页面成功，sitemap 仍为 229 个 canonical URL，内部链接 847/847 可访问。

### 下一步待处理

- Phase 3：继续清理 hub 里残留的 2025 标题、内链和无法验证数据。
- Phase 4：继续补多语言深度本地化，并修正跨语言内链。
