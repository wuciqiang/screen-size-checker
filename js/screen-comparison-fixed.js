...
window.addEventListener('DOMContentLoaded', function() {
    ...
    var comparisonDisplay1 = document.getElementById('comparison-display1');
    var comparisonDisplay2 = document.getElementById('comparison-display2');
    ...
    function updateTitle(size1, aspect1, size2, aspect2, unit) {
        ...
        var title = size1 + ' ' + unitText + ' ' + aspect1Text + ' display vs ' + size2 + ' ' + unitText + ' ' + aspect2Text + ' display';
        comparisonTitle.textContent = title;

        // 更新表头
        display1Header.textContent = size1 + ' ' + unitText + ' ' + aspect1Text;
        display2Header.textContent = size2 + ' ' + unitText + ' ' + aspect2Text;
        comparison1Header.textContent = size1 + ' ' + unitText + ' ' + aspect1Text;
        comparison2Header.textContent = size2 + ' ' + unitText + ' ' + aspect2Text;
    }
    ...
    function updateTables(display1, display2, comparison) {
        ...
        // 更新对比表格
        // 实际比例下的对比
        var actualCompText1 = formatComparison(comparison.actual.diagonalDiff, comparison.actual.areaDiff, false);
        var actualCompText2 = formatComparison(comparison.actual.diagonalDiff, comparison.actual.areaDiff, true);
        compActual1Cell.innerHTML = actualCompText1;
        compActual2Cell.innerHTML = actualCompText2;

        // 4:3比例下的对比
        var comp4x3Text1 = formatComparison(comparison.as4x3.diagonalDiff, comparison.as4x3.areaDiff, false);
        var comp4x3Text2 = formatComparison(comparison.as4x3.diagonalDiff, comparison.as4x3.areaDiff, true);
        comp4x3_1Cell.innerHTML = comp4x3Text1;
        comp4x3_2Cell.innerHTML = comp4x3Text2;

        // 16:9比例下的对比
        var comp16x9Text1 = formatComparison(comparison.as16x9.diagonalDiff, comparison.as16x9.areaDiff, false);
        var comp16x9Text2 = formatComparison(comparison.as16x9.diagonalDiff, comparison.as16x9.areaDiff, true);
        comp16x9_1Cell.innerHTML = comp16x9Text1;
        comp16x9_2Cell.innerHTML = comp16x9Text2;

        // 2.35:1比例下的对比
        var comp235x1Text1 = formatComparison(comparison.as235x1.diagonalDiff, comparison.as235x1.areaDiff, false);
        var comp235x1Text2 = formatComparison(comparison.as235x1.diagonalDiff, comparison.as235x1.areaDiff, true);
        comp235x1_1Cell.innerHTML = comp235x1Text1;
        comp235x1_2Cell.innerHTML = comp235x1Text2;
    }

    // 格式化比较文本
    function formatComparison(diagonalDiff, areaDiff, isSecond) {
        ...
        var diagonalText = diagonalDiff >= 0 ?
            formatNumber(diagonalDiff) + '% larger diagonal' :
            formatNumber(-diagonalDiff) + '% smaller diagonal';

        var areaText = areaDiff >= 0 ?
            formatNumber(areaDiff) + '% larger area' :
            formatNumber(-areaDiff) + '% smaller area';

        return diagonalText + '<br>' + areaText;
    }
    ...
    function updateVisual(display1, display2) {
        ...
        // 设置显示器1的样式
        comparisonDisplay1.style.width = scaledWidth1 + 'px';
        comparisonDisplay1.style.height = scaledHeight1 + 'px';
        comparisonDisplay1.style.left = left1 + 'px';
        comparisonDisplay1.style.top = top1 + 'px';

        // 设置显示器2的样式
        comparisonDisplay2.style.width = scaledWidth2 + 'px';
        comparisonDisplay2.style.height = scaledHeight2 + 'px';
        comparisonDisplay2.style.left = left2 + 'px';
        comparisonDisplay2.style.top = top2 + 'px';
    }
